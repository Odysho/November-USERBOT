...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** ɴᴏᴠᴇᴍʙᴇʀ Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("- M𝙴𝙼𝙸𝙰𝙽 ODY 𐇰.", "t.me/OORRR")]
        ])
...
async def settings(event):
    await event.delete()
    await event.reply(f"⌔∮ انا نوفمبر مساعد {mention}. الشخصي تستطيع الاتصال بي عن طريق هذا البوت",
                             buttons=[
                                     [Button.inline(
                                         "⚜️ حصل على بوت نوفمبر بنفسك ⚜️", data="deployme")]
                             ])
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط اكسس ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="مو مال اصغار")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="مو بلاش")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/OORRR")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @NNEEE", alert=True)
