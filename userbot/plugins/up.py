...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = f"** ⪼ يوجـد تحـديث جديد لسورس نوفمبر ⛅️🌈 ༗.**\n\n\n`{changelog}`\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\nՏøuƦcε πøνεʍβεƦ 🌦⪼ [𝐃𝐄𝐕](t.me/oorrr)"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(f"{txt}\n" "بيانات اعتماد هيروكو غير صالحة لتنصيب نوفمبر")
            return repo.__del__()
        await event.edit(
            "**تنصيب تحديث نوفمبر قيد التقدم ، يرجى الانتظار حتى تنتهي العملية ، وعادة ما يستغرق التحديث من 4 إلى 5 دقائق.**"
        )
        ups_rem.fetch(ac_br)
...
async def upstream(event):
    "بالنسبة لأمر التحديث ، تحقق مما إذا كان بوت نوفمبر محدثًا ، أو قم بالتحديث إذا تم بتحديثه"
    ...
    if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
        return await edit_or_reply(
            event,
            "\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n\n\n** ⪼ اضبط المتغيرات المطلوبة أولاً لتحديث بوت نوفمبر 𓆰،**",
        )
    try:
