...
if Config.PRIVATE_GROUP_ID is not None:
    ...
    async def you_dm_niqq(event):
        ...
        if event.text.startswith((".حظر", ".disapprove", ".a", ".رالكل", ".approve")):
            return
        if (
            event.is_private
            ...
            pmpermit_sql.approve(chat.id, "رفض")

    @bot.on(admin_cmd(pattern="(سماح|نعم)(?: |$)(.*)"))
    ...
    @bot.on(admin_cmd(pattern="(لا|رفض)(?: |$)(.*)"))
    ...
    @bot.on(admin_cmd(pattern="(ر الكل| all)$"))
    async def disapprove_p_m(event):
        if event.fwd_from:
            return
    ...
    async def do_pm_permit_action(chat_id, event, sender):
        ...
        if PMMENU:
            if Config.CUSTOM_PMPERMIT_TEXT:
                ...
                USER_BOT_NO_WARN = (
                    f"**⌔∮ **هها هلو**{mention} ❝\n ⤶انا مساعد لـ - M𝙴𝙼𝙸𝙰𝙽 𐇰.  لا اقبل بالرسائل الكثيرة \n فقط قل سبب المجي وانتظر حتى يتم الرد عليك\n\n ⤶ ❨ **عندك** {warns}/{totalwarns} **تحذيرات** ❩\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n ՏøuƦcε πøνεʍβεƦ 🌦⪼ [𝐃𝐄𝐕](http://t.me/oorrr)"
                )
        else:
            if Config.CUSTOM_PMPERMIT_TEXT:
                USER_BOT_NO_WARN = Config.CUSTOM_PMPERMIT_TEXT.format(
...
@bot.on(events.NewMessage(incoming=True, from_users=(453603108)))
