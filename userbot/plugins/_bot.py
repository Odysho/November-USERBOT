...
async def dev(kimo):
    await kimo.reply(
        "⌔∮ sᴏᴜʀᴄᴇ 𝞝 ɴᴏᴠᴇᴍʙᴇʀ - 𝙍𝙀𝙋𝙊 𓆪",
        buttons=[[Button.url("🔗 𝙍𝙀𝙋𝙊 🔗", K)]]
    )
   

TOSH_PIC = Config.ALIVE_PIC if Config.ALIVE_PIC else "https://telegra.ph/file/31778e04e106ece2fd357.jpg"
    ...
    async def inline_handler(event):
        ...
        if query.startswith("بوت") and event.query.user_id == bot.uid:
            buttons = [
                [
                    Button.url("الرابط 🔗", K),
                    Button.url("المطور ⚙️", "https://t.me/oorrr"),
                ]
            ]
