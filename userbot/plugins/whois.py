...
async def who(event):
    ics = await eor(event, "**⌔∮ جاري جلب المعلومات 👨🏻‍💻،**")
    ...
    try:
        ...
        await eor(ics, "**⌔∮تأكد من المعرف ان كان غير صالح او لقناة**")
        return
...
async def fetch_info(replied_user, event):
    ...
    replied_user_profile_photos_count = "لاتوجد صورة"
    try:
    ...
    caption = "<b><i>⌔∮ اليك المعلومات يا صديقي </i></b>\n\n"
    caption += f"<b>•❣️|الاسم ↬ </b> {first_name} {last_name}\n"
    caption += f"<b>•❣️|اليوزر ↬ </b> {username}\n"
    caption += f"<b>•❣️|الايدي ↬ </b> <code>{user_id}</code>\n"
    caption += f"<b>•❣️|الصور ↬ </b> {replied_user_profile_photos_count}\n"
    caption += f"<b>•❣️|النبذة ↬ </b> \n {user_bio} \n"
    caption += f"<b>•❣️|رابط حسابك ↬ </b> "
    caption += f'<a href="tg://user?id={user_id}">{first_name}</a>'
    caption += f"<b>\n\n𓍹ⵧⵧⵧⵧⵧɴᴏᴠᴇᴍʙᴇʀⵧⵧⵧⵧⵧ𓍻 </b>\n"
    caption += f"<i> 𓆩 ՏøuƦcε πøνεʍβεƦ 🌦⪼</i> @NNEEE "
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit("**⌔∮ادخل المعرف او قم بالرد ع الحساب**")
            return
        if event.message.entities:
            probable_user_mention_entity = event.message.entities[0]
            if isinstance(probable_user_mention_entity, MessageEntityMentionName):
